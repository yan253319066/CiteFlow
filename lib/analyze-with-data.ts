...
import type { ScrapeResult } from './client-scrape';
...
export async function analyzeWithData(url: string, scrapeData: ScrapeResult): Promise<Record<string, unknown>> {
  const cacheKey = `report:${url}`;
  const cached = cacheGet<Record<string, unknown>>(cacheKey);
  if (cached) return { ...cached, cached: true };

  if (scrapeData.error && !scrapeData.title && scrapeData.wordCount === 0) {
    const errorInfo = getErrorInfo(scrapeData.error);
    const errorResult = {
      score: 0,
      breakdown: { aiVisibility: 0, faqCoverage: 0, entityClarity: 0, authority: 0 },
      ...
      error: scrapeData.error || 'Website inaccessible',
      errorCode: errorInfo.code,
    };
    cacheSet(cacheKey, errorResult, 5 * 60 * 1000);
    return errorResult;
  }

  const baseScore = calculateBaseScore(scrapeData);
  const siteData = formatSiteData(url, scrapeData);
...
function getErrorInfo(errorMessage?: string): ErrorInfo {
  ...
  if (errorLower.includes('cors')) {
    return {
      code: 'CORS_ERROR',
      missing: ['Website blocks cross-origin requests'],
      suggestions: [
        'This website prevents direct browser access from other domains',
        'Try analyzing from the website itself or contact the website owner',
      ],
      summary: 'Analysis failed: CORS policy prevents access',
    };
  }
  ...
  if (errorLower.includes('forbidden') || errorLower.includes('access') || errorLower.includes('403')) {
    return {
      code: 'AUTH_ERROR',
      ...
      suggestions: [
        ...
        'Some hosting providers block scrapers',
      ],
