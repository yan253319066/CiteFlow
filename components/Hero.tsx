...
export function Hero() {
  ...
  const { ref: scannerRef, isInView: scannerVisible } = useInView(0.1);
  ...
  return (
    <section className="relative pb-20 px-6 overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto text-center z-10">
        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 bg-white/5 text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-8 ${anim(0)}`}>
          <Sparkles className="w-3 h-3 text-primary" />
          <span>{t?.badge || 'Enterprise AI Brand Service'}</span>
        </div>

        <h1 className={`hero-title mb-6 ${anim(1)}`}>
          {t ? (
        ...
        <p className={`text-xl text-slate-400 font-light max-w-2xl mx-auto mb-10 ${anim(2)}`}>
        ...
        <div className={`flex flex-col md:flex-row items-center justify-center gap-4 mb-8 ${anim(3)}`}>
        ...
        <div className={`flex items-center justify-center gap-3 text-sm ${anim(4)}`}>
      ...
      <div id="scanner-section" ref={scannerRef} className="max-w-5xl mx-auto text-center z-10 mt-20 pt-12 border-t border-white/5">
        <p className={`text-xs text-slate-500 font-semibold uppercase tracking-widest mb-2 ${scannerVisible ? 'animate-fade-in-up' : ''}`}>
          {t?.scannerLabel || 'Free AI Visibility Scanner \u2014 A Free Diagnostic for Your Brand'}
        </p>
        <p className={`text-sm text-slate-500 mb-6 ${scannerVisible ? 'animate-fade-in-up stagger-delay-1' : ''}`}>
          {t?.scannerHint || 'Enter any URL. Get a 0-100 score. No signup required.'}
        </p>
        <form onSubmit={handleAnalyze} className={`relative max-w-xl mx-auto w-full ${scannerVisible ? 'animate-fade-in-up stagger-delay-2' : ''}`}>
